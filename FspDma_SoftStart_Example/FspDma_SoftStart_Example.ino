...
void setup() {
  ...
  extend_cfg.activation_source = ELC_EVENT_NONE;  // From Table 13.4 in RA4M1 Hardware User's Manual (use ELC_EVENT_NONE for software start)
...
void loop() {
  ...
  // Send an 'x' from the Serial Monitor to trigger a transfer by soft start. 
  if(Serial.read() == 'x'){
    // Use TRANSFER_START_MODE_REPEAT and it will soft start again after each transfer
    // Use TRANSFER_START_MODE_SINGLE to run one transfer and stop
    R_DMAC_SoftwareStart(&ctrl, TRANSFER_START_MODE_SINGLE); 
  }
  Serial.print("  -");
